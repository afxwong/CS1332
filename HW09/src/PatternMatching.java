...
public class PatternMatching {
    ...
    public static List<Integer> boyerMoore(CharSequence pattern,
        ...
        while (i <= text.length() - pattern.length()) {
            ...
            if (j < 0) {
                ...
                shift = lastOccurrence.get(text.charAt(i + j));
                if (shift == null) {
                    shift = -1;
                }
                if (shift < j) {
    ...
    public static List<Integer> rabinKarp(CharSequence pattern,
        ...
        for (int i = pattern.length() - 1; i >= 0; i--) {
            patternHash += pattern.charAt(i) * startbase;
            textHash += text.charAt(i) * startbase;
            if (i != 0) {
                startbase *= BASE;
            }
        }
        ...
        while (i <= text.length() - pattern.length()) {
            ...
            if (i <= text.length() - pattern.length()) {
                textHash = rollHash(textHash, text, i - 1, i + pattern.length() - 1, startbase);
            }
        }
        return matches;
    }
    ...
    /**
     * Helper method for rolling hash.
     * @param oldHash previous hash
     * @param charSequence the text
     * @param oldindex index to remove
     * @param newindex index to add
     * @param base BASE^(pattern.length - 1)
     * @return new hash
     */
    private static int rollHash(int oldHash, CharSequence charSequence, int oldindex, int newindex, int base) {
        return (oldHash - charSequence.charAt(oldindex) * base) * BASE + charSequence.charAt(newindex);
    }
}
